...
public class PostOffice {
    public static void main(String[] args) {
        ...
        String regexCapitals = "([#$%*&]+)(?<capitals>[A-Z]+)\\1";
        String regexLength = "(?<letter>[\\d]{2}):(?<length>[0-9][0-9])";

        Pattern patternCapitals = Pattern.compile(regexCapitals);
        ...
        if (matcherCapitals.find()) {
            capitals = Arrays.stream(matcherCapitals.group("capitals").split("")).collect(Collectors.toList());
        }
